var OriginalToken = artifacts.require("./OriginalTokenMock");
...
contract('OriginalToken', function([author, friend, user]) {
    let token;
    ...
    it("the owner can change the key", async function() {
      ...
      assert.equal(await token.encryptedKey(),"new key")
    })
    ...
    it("the owner can change the link", async function() {
      ...
      assert.equal(await token.link(),"https://www.youtube.com/watch?v=tas0O586t80")
    })
